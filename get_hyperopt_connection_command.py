...
def get_hyperopt_worker_command(instance):
    ...
    remote_command = "cd ddpg_agent && git pull && ./kill_hyperopt_mongo_workers.sh && if [ -f \'hyperopt-mongo-worker-*.log\' ]; then rm hyperopt-mongo-worker-*.log ; fi ; ./start_hyperopt_mongo_workers.sh %i"%(instance['cpu_cores']/2)
    return get_ssh_connect_command(instance)+' "%s"'%remote_command

for instance in get_instances():
    if instance['actual_status']=='running':
        print(get_hyperopt_worker_command(instance)+" && "+get_ssh_connect_command(instance)+ " -R 27017:localhost:27017 \"cd ddpg_agent && ./tail_logs.sh 2 \"")
#         print(get_ssh_connect_command(instance)+" \"cd ddpg_agent && git pull && ./kill_hyperopt_mongo_workers.sh && if [ -f \'hyperopt-mongo-worker-*.log\' ]; then rm hyperopt-mongo-worker-*.log ; fi\" ; "+get_hyperopt_worker_command(instance)+" && "+get_ssh_connect_command(instance)+ " -R 27017:localhost:27017 \"cd ddpg_agent && while [ 1 ]; do tail -n5 hyperopt-mongo-worker-1.log ; echo ; tail -n5 hyperopt-mongo-worker-2.log ; echo ; tail -n5 hyperopt-mongo-worker-3.log ; echo ; tail -n5 hyperopt-mongo-worker-4.log ; echo ; sleep 1; done \" ")
        print()
